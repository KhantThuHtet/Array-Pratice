const inventors = [
  { first: "Albert", last: "Einstein", year: 1879, passed: 1955 },
  { first: "Isaac", last: "Newton", year: 1643, passed: 1727 },
  { first: "Galileo", last: "Galilei", year: 1564, passed: 1642 },
  { first: "Marie", last: "Curie", year: 1867, passed: 1934 },
  { first: "Johannes", last: "Kepler", year: 1571, passed: 1630 },
  { first: "Nicolaus", last: "Copernicus", year: 1473, passed: 1543 },
  { first: "Max", last: "Planck", year: 1858, passed: 1947 },
  { first: "Katherine", last: "Blodgett", year: 1898, passed: 1979 },
  { first: "Ada", last: "Lovelace", year: 1815, passed: 1852 },
  { first: "Sarah E.", last: "Goode", year: 1855, passed: 1905 },
  { first: "Lise", last: "Meitner", year: 1878, passed: 1968 },
  { first: "Hanna", last: "Hammarström", year: 1829, passed: 1909 },
];


const people = ["Bernhard, Sandra","Bethea, Erin","Becker, Carl","Bentsen, Lloyd","Beckett, Samuel","Blake, William","Berger, Ric","Beddoes, Mick","Beethoven, Ludwig","Belloc, Hilaire","Begin, Menachem","Bellow, Saul","Benchley, Robert","Blair, Robert","Benenson, Peter","Benjamin, Walter","Berlin, Irving","Benn, Tony","Benson, Leana","Bent, Silas","Berle, Milton","Berry, Halle","Biko, Steve","Beck, Glenn","Bergman, Ingmar","Black, Elk","Berio, Luciano","Berne, Eric","Berra, Yogi","Berry, Wendell","Bevan, Aneurin","Ben-Gurion, David","Bevel, Ken","Biden, Joseph","Bennington, Chester","Bierce, Ambrose","Billings, Josh","Birrell, Augustine","Blair, Tony","Beecher, Henry","Biondo, Frank",
];

// 1. Filter the list of inventors for those who were born in the 1500's
const fifteen = inventors.filter(el => el.year < 1600 && el.year > 1500);
// console.table(fifteen);

// 2. Give us an array of the inventor first and last names
const nameList = inventors.map(el => `${el.first} ${el.last}`);
// console.log(nameList);


// 3. Sort the inventors by birthdate, oldest to youngest
const sortBirthday = inventors.sort( (x,y) => x.year - y.year);
// console.table(sortBirthday);

// 4. How many years did all the inventors live?
const totalYearOfLive = inventors.reduce((x,y) => {
    return x + (y.passed - y.year);
}, 0);
// console.log(yearOfLive);

//Additional - How many years did each inventor lives?
const yearOfLive = inventors.map(el=>{
    return el.live = el.passed - el.year;
    
});
// console.log(yearOfLive);
// 5. Sort the inventors by years lived
const sortByYearsLived = inventors.sort((x,y) =>{
    return (y.passed - y.year) - (x.passed - x.year);

});
// console.log(sortByYearsLived);

// 6. create a list of Boulevards in Paris that contain 'de' anywhere in the name
// 6. create a list of Boulevards in Paris that contain 'de' anywhere in the name
    // https://en.wikipedia.org/wiki/Category:Boulevards_in_Paris

    // const category = document.querySelector('.mw-category');
    // const links = Array.from(category.querySelectorAll('a'));
    // const de = links
    //             .map(link => link.textContent)
    //  


// 7. sort Exercise
const sortPeople = people.sort((x,y) => y.localeCompare(x));
// console.log(sortPeople);

// 8. Reduce Exercise
const data = ['car', 'car', 'truck', 'truck', 'bike', 'walk', 'car', 'van', 'bike', 'walk', 'car', 'van', 'car', 'truck', 'pogostick'];

const result = data.reduce((x,y) => {
  if (!x[y]) {
    x[y] = 0;
  }
  x[y]++;
  return x;
}, {});

console.log(result);

